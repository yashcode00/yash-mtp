...
output_main="${log_dir}extractData.log"

eval "$(conda shell.bash hook)" &> /nlsasfs/home/nltm-st/sujitk/yash-mtp/logs/wav2vec2/error.txt
wget --no-proxy https://asr.iitm.ac.in/SPRING_INX/data/SPRING_INX_Kannada_R1.tar.gz &> wget.log &
...
python /nlsasfs/home/nltm-st/sujitk/yash-mtp/extract.py &> "$output_main" &
