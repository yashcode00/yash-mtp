...
def CreateIfNot(parent, path):
    if path not in os.listdir(parent):
        ...
        print(f"Created new directory {os.path.join(parent,path)}")
        return os.path.join(parent,path)
    print(f"Directory already exist namely {os.path.join(parent,path)}")
    return os.path.join(parent,path)
                ...
                for audioFile in os.listdir(pathHere2):
                    ...
                    if not audioFile.startswith('.'):
                        ...
                        try:
                            ...
                            chunks = break_audio_into_chunks(finalPath, chunk_size=2000)  # 2 second chunks
                            f = audioFile.split(".")[0]
